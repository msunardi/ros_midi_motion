...
NOTE_RANGE = 127
RANGE_OF_MOTION = 3000	# for asc16
UP_BOUND = 10
LOW_BOUND = 2
...
def analyzePhrase(phrase):	
	...
	for bi in phrase:	# for each beat in the phrase
		...
		last_note = curr_note
		#beat_tick = min([j[0] for j in bi[2]])
		...
		if notes:	# if there are note-on events within the beat..
			...
			if is_this_rest:
				...
				phrase_start = min([j[0] for j in bi[2]])
			...
			curr_note = next_note
			note_start= min([i[0] for i in notes])
			continue	# Keep going...
		...
		elif not notes and (beat_tick > phrase_end) and tmp_notes != []:	# If there are no note events, and after SUSTAIN state...
...
def analyzePhrase2(phrase, tempo=0.5):
	rest_count = 0
	rest_start = 0
	rest_end = 0
	rest_energy = 0
	...
	tmp_ctime = []
	tmp_notes = []
	tp = lambda x: x/384.*tempo  # << already in seconds
	poses = [[[1,3,0],[2,13,0]],[[1,10,0],[2,10,0]]]
	for bi in phrase:
		tmp_notes = bi[2]
		
		if bi[1] == 0:
			tx = bi[0]	
			#print "this is beat is silent.",
		else:
			tx = min([x[0] for x in tmp_notes])
			#print "this is note event.",
		t = tp(tx)	
		#print "At:", tx,"(",int(t)/60,":",t%60,") "
		tmp_notes = []
		if bi[1] != 0:
			tmp_ctime.append(t)
			if pos:	tmp_contour.append(poses[0])
			else: tmp_contour.append(poses[1])
			pos = not pos
				
	#print "tmpctimes=", tmp_ctime
	#print "tmpcontour=", tmp_contour
	print len(tmp_ctime), len(tmp_contour)
	return tmp_ctime, tmp_contour
	
def analyzePhrase3(phrase, tempo=0.5):
	
	head_robot_home = {'1': 1636, '2': 2560, '3': 3328, '4': 1636, 'mid': 1636}
	rest_count = 0
	rest_start = 0
	rest_end = 0
	rest_energy = 0
	...
	for bi in phrase:
		...
		if bi[1] != 0:
			...
			# For servo1, since HOME is center can do + or -			
			if pos:	
				d_pose1 = head_robot_home['1'] - disp
				
			else:
				d_pose1 = head_robot_home['1'] + disp
			
			d_pose1 = limitpos(abs(d_pose1))
			pose1 = [1, int(d_pose1)/256, int(d_pose1)%256]
			pose4 = [4, int(d_pose1)/256, int(d_pose1)%256]
			
			# For servo2, since HOME is max, only do -
			d_pose2 = limitpos(abs(head_robot_home['2'] - disp))
			pose2 = [2, int(d_pose2)/256+1, int(d_pose2)%256]
			pose3 = [3, int(d_pose2)/256+1, int(d_pose2)%256]
			
			tmp_contour.append([pose1, pose2, pose3, pose4])
			pos = not pos
				
	#print "tmpctimes=", tmp_ctime
	#print "tmpcontour=", tmp_contour
	print len(tmp_ctime), len(tmp_contour)
	contour = [i[0][1] for i in tmp_contour]
	fftcontour = numpy.fft.fft(contour)
	#fftcontour[:-100]=0
	ifftcontour = numpy.fft.ifft(fftcontour)
	
	#plot(tmp_ctime, ifftcontour)
	#show()
	
	#tmp_contour = [[[1,int(abs(i)),0]] for i in ifftcontour]
	#print tmp_contour
	
	return tmp_ctime, tmp_contour
	
def limitpos(pos, uplimit=2660, lolimit=1000):
	if pos > uplimit:
		return uplimit
	elif pos < lolimit:
		return lolimit
...
def findNotes(notesphrase, base=60, d_range=1400):
	# notephrase format: [ (note1): [tick, len, note#, von, voff]
	#					   (note2): [tick, len, note#, von, voff]... ]
	# Returns the displacement for next movement
	# base = 60 == middle C note
	# d_range = 1400 == ~60 degrees for max displacement
	if sum([len(i) for i in notesphrase]) == len(notesphrase)*5:
		note_times = [i[0] for i in notesphrase]
		note_len = [i[1] for i in notesphrase]
		note_num = [i[2] for i in notesphrase]
		note_ons = [i[3] for i in notesphrase]
		max_ons = max(note_ons)
		max_index = note_ons.index(max_ons)
		note = note_num[max_index]
		max_note = max(note_num)
		
		pos = abs(float(note)-base)/(abs(float(max_note)-base)+1)*d_range
		
		#print "notes: ", note_num, " note_ons: ", note_ons
		#print "note:", note, "Von:", max_ons, "max_note:", max_note, "pos:", pos
		return pos
		
	else:
		print "invalid entry in notesphrase"
		return 0

def executemotion(timing, motiondata, tempo=0.5):
	start = time.time()
	timing_=iter(timing)
	motiondata_=iter(motiondata)
	current = 0
	threshold = tempo
	# playingwithasc16.serie([[81,150],[82,50]])
	print "TIMING: ", timing
	print "MOTIONDATA: ", motiondata
	for i in timing:			
		print "t=", int(i)/60.0, ":", i%60		
		while i - current > threshold:
			current = time.time() - start
			#print "time:",current
		# playingwithasc16.serie_simple(motiondata_.next())
		print "MOVE!"
...
def formatforasc16(data, servo_id=5, speed=50):
	tmp_data = [[80+servo_id,speed]]
	t = 0
	val = 0
...
def processpos(data, servo_id):
	...
	for i in data:
		...
		if i['rest'] > 0:
			...
			p = i['delta'] * i['energy']
			#print p
			if servo_id in [5,6,7,8]:	# << for certain servos which values are inverted: bigger value = lowest position
				if p < -4000: p = 4000
				elif p > 4000: p = -4000
				else: p = 4000-p
			else:
				if p < -4000: p = -4000		# << limit the range of motion
				elif p > 4000: p = 4000
...
def processpos2(data, servo_id, beat):
	...
	for i in data:
		if i['phrase']:		# If it's a (melodic) phrase...
			...
			p_data_ = [converts(int(float(p)/max(p_data)*1500.)) for p in p_data]
			...
			tmp.extend(p_data_)
			if i['contour_time'] != []: timing.append(max(i['contour_time']))
		
		else:				# Otherwise it's a rest (phrase).
			tmp.extend([i['rest_duration']*beat])
			timing.append(i['rest_start'])
...
def waitToComplete(data):
	# assumed data in the form of asc16 move commands
	tmp = []
	for i in data:
		tmp.append(i)
		if i[0] in range(1,17):			
			tmp.append([180+i[0]])		
	return tmp
	
def addDelay(data,delay=100):	
	# assumed data in the form of asc16 move commands
	tmp = []
	for i in data:
		tmp.append(i)
		if i[0] in range(1,17):			
			tmp.append([111, delay])		
	return tmp

def repeatBeat(data, repetition=1):
	tmp = []
...
def main():
	PATH = '../temp/lullaby_of_birdland_part_quantz.med'
	PATH2 = '../temp/lullaby_of_birdland_drum2.med'
	#PATH2 = '../temp/lullaby_of_birdland_part_quantz.med'
	#PATH = '../temp/beethoven-5th_part1.med'
	#PATH2 = '../temp/beethoven-5th_part1.med'
	ch_base = 2
	ch_neck_bottom = 3
	ch_neck_mid = 5
	...
	khr1home = [[1,0,0],[2,0,0],[3,10,0],[4,5,0],[5,13,0],[6,11,0],[7,3,0],[8,7,0],[9,6,0],[10,6,0],[11,5,0],[12,6,0]]
	...
	KHR1channels={'l_shoulder_x':1,
				  ...
				  'l_hip':11,
				  'r_hip':12}
	KHR1gestures={'other': {'home': {'l_shoulder_x':[KHR1channels['l_shoulder_x'],0,0],
						   'l_shoulder_y':[KHR1channels['l_shoulder_y'],0,0],
						   'l_elbow_low':[KHR1channels['l_elbow_low'],5,0],
						   'l_elbow_mid':[KHR1channels['l_elbow_mid'],10,0],
						   'r_shoulder_x':[KHR1channels['r_shoulder_x'],13,0],
						   'r_shoulder_y':[KHR1channels['r_shoulder_y'],11,0],
						   'r_elbow_low':[KHR1channels['r_elbow_low'],7,0],
						   'r_elbow_mid':[KHR1channels['r_elbow_mid'],3,0],
						   'head_pan': [KHR1channels['head_pan'],6,0],
						   'head_pitch': [KHR1channels['head_pitch'],6,0],
						   'l_hip': [KHR1channels['l_hip'],5,0],
						   'r_hip': [KHR1channels['r_hip'],6,0]},
						   },
					'deictic': {'right': {'up': [[KHR1channels['r_shoulder_x'],0,0]],
										  'down': [[KHR1channels['r_shoulder_x'],13,0]],
										  'left': [[KHR1channels['r_shoulder_x'],5,0],[KHR1channels['r_elbow_mid'],7,0],[KHR1channels['r_elbow_low'],13,0]],
										  'right': [[KHR1channels['r_shoulder_y'],5,0],[KHR1channels['r_elbow_low'],7,0]],
										  'forward': [[KHR1channels['r_shoulder_x'],5,0]],
										  },
								'left': {'up': [[KHR1channels['l_shoulder_x'],13,0]],
										  'down': [[KHR1channels['l_shoulder_x'],0,0]],
										  'left': [[KHR1channels['l_shoulder_x'],6,0],[KHR1channels['l_elbow_mid'],5,0]],
										  'right': [[KHR1channels['l_shoulder_y'],5,0],[KHR1channels['l_elbow_low'],0,0],[KHR1channels['l_elbow_mid'],7,0]],
										  'forward': [[KHR1channels['l_shoulder_x'],5,0]],
										  },
							   },
					'iconic': {'right': {'open': [[KHR1channels['r_elbow_mid'],7,0]],
										 'box':[[KHR1channels['r_shoulder_x'],10,0],[KHR1channels['r_shoulder_y'],2,0],[KHR1channels['r_shoulder_x'],3,0],[KHR1channels['r_shoulder_y'],0,0]]
										},
							   'left': {'open': [[KHR1channels['l_elbow_mid'],13,0]],
										'box':[[KHR1channels['l_shoulder_x'],10,0],[KHR1channels['l_shoulder_y'],2,0],[KHR1channels['l_shoulder_x'],3,0],[KHR1channels['l_shoulder_y'],0,0]]
										}
							  },
				    'beat': {'right': {'updown': [[KHR1channels['r_shoulder_x'],0,0],[KHR1channels['r_shoulder_x'],13,0]]
				                      },
				             'left': {'updown': [[KHR1channels['l_shoulder_x'],13,0],[KHR1channels['l_shoulder_x'],0,0]]
				                      }
				            },
				    'head': {'look': {'up': [[KHR1channels['head_pitch'],13,0]],
									  'down': [[KHR1channels['head_pitch'],4,0]],
									  'left': [[KHR1channels['head_pan'],0,0]],
									  'right': [[KHR1channels['head_pan'],13,0]]
									  },
							 'yes': [[KHR1channels['head_pitch'],9,0],[180+KHR1channels['head_pitch']],[KHR1channels['head_pitch'],4,0]],
							 'no': [[KHR1channels['head_pan'],0,0],[180+KHR1channels['head_pan']],[KHR1channels['head_pan'],13,0]]
							 }
				 }
	#playingwithasc16.init(port='COM4')
	#playingwithasc16.go([0,0,0])			 
	#playingwithasc16.serie([v for k,v in KHR1gestures['other']['home'].iteritems()])
	#import pickle
	...
	#chan3 = processpos2(analysis_drum, servo_id=10, beat=timeperbeat)
	#chan5 = processpos(analysis, servo_id=5)#, beat=timeperbeat)
	#print "chan3len: ", len(chan3)
	chan5, time5 = processpos2(analysis, servo_id=1, beat=timeperbeat)
	chan7, time7 = processpos2(analysis, servo_id=3, beat=timeperbeat)
	#chan9 = processpos2(analysis, servo_id=4, beat=timeperbeat)
	#chan11 = processpos2(analysis_drum, servo_id=5, beat=timeperbeat)
	#chan13 = processpos2(analysis, servo_id=7, beat=timeperbeat)
	#chan15 = processpos2(analysis_drum, servo_id=2, beat=timeperbeat)
	#chan17 = processpos2(analysis, servo_id=8, beat=timeperbeat)
	#chan19 = processpos2(analysis_drum, servo_id=9, beat=timeperbeat)
	#chan21 = processpos2(analysis_drum, servo_id=6, beat=timeperbeat)
	#chan23 = processpos2(analysis_drum, servo_id=11, beat=timeperbeat)
	#chan25 = processpos2(analysis_drum, servo_id=12, beat=timeperbeat)
	
	#-PLOT THE MOTION DATA:-------------
	#print "chan5:", chan5[:10]	
	motiondata = chan5[:]
	y = []
	prev_pos = 0
	for i in motiondata:
		print i,
		if isinstance(i, float):
			for r in range(int(i)+1): y.append(prev_pos)
		elif isinstance(i, list):
			zip = i[0]*255+i[1]
			y.append(zip)
			prev_pos = zip
	l = len(y)
	ffty = numpy.fft.rfft(y,l)
	ffty[100:] = 0
	iffty = numpy.fft.irfft(ffty)
	#fftfreqy = numpy.fft.fftfreq(l)
	
	#plot(iffty)
	#show()
	
	newmotion = [converts(int(i)) for i in iffty]
	#newmotion = [converts(int(i)) for i in y]
	
	#print "Newmotion:", 
	#for i in newmotion[:20]: print i
	newmotion_ = formatforasc16(newmotion, servo_id=1)
	#print "Formatted newmotion:", 
	#for i in newmotion_[:20]: print i
	#pid = Popen(["python", "playmidi.py"])	# << uncomment to midi
	#time.sleep(5)
	#playingwithasc16.seriex(newmotion_, delayx=.01, tempox=284000/len(newmotion_))
	
	#-----------------------------------
	#print "time5:", time5
	timecon = lambda x: x/384*.5
	time5_ = [timecon(x) for x in time5]
	#print "time5 (in sec):", time5_
	print "len surface vs len time:", len(chan5), len(time5_)
	"""
	...
	chan3_ = formatforasc16(chan3, servo_id=10)
	"""
	chan5_ = formatforasc16(chan5, servo_id=1)
	
	chan7_ = formatforasc16(chan7, servo_id=2)	# servo_id = 3 (old value)
	"""
	chan9_ = formatforasc16(chan9, servo_id=4, speed=100)	
	chan11_ = formatforasc16(chan11, servo_id=5)
	chan13_ = formatforasc16(chan13, servo_id=7)
	chan15_ = formatforasc16(chan15, servo_id=2)
	chan17_ = formatforasc16(chan17, servo_id=8)
	chan19_ = formatforasc16(chan19, servo_id=9)
	chan21_ = formatforasc16(chan21, servo_id=6)
	chan23_ = formatforasc16(chan23, servo_id=11)
	chan25_ = formatforasc16(chan25, servo_id=12)
	"""
	#molist = combine(combine(combine(combine(combine(chan5_,chan3_), combine(chan7_,chan9_)), combine(combine(chan11_, chan13_), combine(chan15_, chan17_))), combine(chan19_, chan21_)), combine(chan23_,chan25_))
	molist = combine(chan5_, chan7_)
	#for i in molist:
	#	print i
	"""
	...
	#molist = []
	#for a in range(5):
	#	molist.extend(gestures['beat']['updown'])
	#print "MOLIST:", molist
	...
	#pid = Popen(["python", "playmidi.py"])	# << uncomment to midi
	#time.sleep(4)  # << uncomment to make music play on time
	#playingwithasc16.seriex(molist, delayx=.5, tempox=284000/len(molist))		# << uncomment to run on robot
	#playingwithasc16.serie([KHR1gestures['deictic']['right']['right'][0], KHR1gestures['deictic']['left']['up'][0]])
	#time.sleep(3)
	#data = addDelay(repeatBeat(KHR1gestures['head']['yes'],3),100)
	#datax = combine(data, waitToComplete(KHR1gestures['iconic']['left']['box']))
	#playingwithasc16.serie([v for k,v in KHR1gestures['other']['home'].iteritems()])
	#playingwithasc16.serie(waitToComplete(repeatBeat(KHR1gestures['iconic']['left']['box'],3)))
	#playingwithasc16.serie(datax)
	#playingwithasc16.serie(waitToComplete(repeatBeat(KHR1gestures['head']['yes'],3)))
	#playingwithasc16.closeSerial()		# << uncomment when playing
	#playingwithasc16.serie([[3,10,0],[5,10,0]])
	#playingwithasc16.seriex(molist, delayx=.7, tempox=700)
	#while True:
	#	continue
	#pos = 0
	#for i in analysis:
